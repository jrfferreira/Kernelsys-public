<?php
	include_once('../app.access/TOccupant.class.php');

 /*
     TChartCore - a PHP class to build charts!
 ...
 class TChartCore
  {
   ...
   var $Errors         = NULL;
   var $ErrorFontName  = "app.files/Fonts/pf_arma_five.ttf";
   ...
   var $tmpFolder        = "app.tmp/";
   var $MapID            = NULL;

   /* This function create the background picture */
   function TChartCore($XSize,$YSize)
    {
     $this->XSize   = $XSize;
     $this->YSize   = $YSize;
     ...
     $this->setFontProperties("../".TOccupant::getPath()."app.files/Fonts/tahoma.ttf",8);
    }
   ...
   function getImageMap($MapName,$Flush=TRUE)
    {
     ...
     $Values   = "../".TOccupant::getPath().$this->tmpFolder.$MapName;
     $Value    = split("\?",$Values);
     $FileName = $Value[0];
   ...
   function SaveImageMap()
    {
     ...
     $Handle = fopen("../".TOccupant::getPath().$this->tmpFolder.$this->MapID, 'w');
     if ( !$Handle )
      {
