...
class TBiblioteca {
    ...
    */
    public function getProdutoBiblioteca() {
        try {
            ...
            $criterio->add(new TFilter("tabela","=","dbpatrimonios_livros"));
            $criterio->add(new TFilter("unidade","=",$unidade->codigo));
            $ret = $obTDbo->select("codigo",$criterio);

            if($obProduto = $ret->fetchObject()) {
                return $obProduto->codigo;
            }else {
                $insert["label"] = "Multa Biblioteca";
                $insert["descricao"] = "Multa por atraso na devolução de livros.";
                $insert["tabela"] = "dbpatrimonios_livros";
    ...
    */
    public function getHistoricoPessoa($codigopessoa) {
        try {
            if($codigopessoa) {
                $this->obTDbo->setEntidade(TConstantes::VIEW_PESSOAS_LIVROS);
                $criterio = new TCriteria();
                $criterio->add(new TFilter("codigopessoa","=",$codigopessoa));
    ...
    */
    public function getHistoricoLivro($codigolivro) {
        try {
            if($codigolivro) {
                $this->obTDbo->setEntidade(TConstantes::DBPESSOAS_LIVROS);
                $criterio = new TCriteria();
            ...
            if($codigolivro) {
                ...
                if($obLivro->codigolocador == "") {
                    if($this->getSituacaoPessoa($codigopessoa)) {
                        ...
                        $vet["codigolivro"] = $codigolivro;
                        $vet["codigopessoa"] = $codigopessoa;
                        $vet["situacao"] = "1";
                        $vet["ativo"] = "1";
                        ...
                        $obTDbo = new TDbo(TConstantes::DBPESSOAS_LIVROS);
                        $criterio = new TCriteria();
                        $criterio->add(new TFilter("codigolivro","=",$codigolivro));
                        $criterio->add(new TFilter("codigopessoa","=",$codigopessoa));
                        $criterio->add(new TFilter("situacao","=","2"));
                        $retReserva = $obTDbo->select("codigo,situacao",$criterio);

                        while($obRetorno = $retReserva->fetchObject()) {
                            $retDados = $obTDbo->delete($obRetorno->codigo,"codigo");
                        }
                        $retDados = $obTDbo->insert($vet);
                        $vet["codigo"] = $retDados["codigo"];
            ...
            if($codigolivro) {
                ...
                if($obLivro->codigolocador != "") {
                    if($codigopessoa) {
                        ...
                        $criterio->add(new TFilter("codigolivro","=",$codigolivro));
                        $criterio->add(new TFilter("codigopessoa","=",$codigopessoa));
                        $criterio->add(new TFilter("situacao","=","1"));
                        ...
                        if($obDevolucao) {
                            if($vet["confirmacaoentrada"] > $obDevolucao->previsaoentrada) {
                                ...
                                $obprodBiblioteca = $prodBiblioteca->getProduto('codigo', $codigoProduto);
                                //==========================================
                            ...
                            $tobTDbo = new TDbo(TConstantes::DBPESSOAS_LIVROS);
                            $criterio2 = new TCriteria();
                            $criterio2->add(new TFilter("codigo","=",$obDevolucao->codigo));
                            $retDados = $tobTDbo->update($vet,$criterio2);

                            $vet['codigotrasacao'] = $codigotransacao;
                            $vet['codigoconta']    = $codigoContaM;
                            $vet['multa']          = $valortotal;
                            $vet["codigo"] = $obDevolucao->codigo;
   ...
   public function setReserva($codigopessoa,$codigolivro) {
        ...
        $dbo = new TDbo(TConstantes::DBPESSOAS_LIVROS);
        $data['codigopessoa'] = $codigopessoa;
    ...
    */
    ...
    public function viewSetDevolucao($codigoRegistro) {
        ...
        $this->obTDbo->setEntidade(TConstantes::DBPESSOAS_LIVROS);
        $criterio = new TCriteria();
        $criterio->add(new TFilter("codigo","=",$codigoRegistro));
        $retDevolucao = $this->obTDbo->select("*",$criterio);
        $obDevolucao = $retDevolucao->fetchObject();

        $obPessoa = $this->getHistoricoPessoa($obDevolucao->codigopessoa);
        ...
        $obFieds->geraCampo("Cod.:", 'codigo', "TEntry", '');
        $obFieds->setProperty('codigo', 'disabled', 'disabled');
        $obFieds->setValue("codigo", $obPessoa->pessoa->codigo);

        $obFieds->geraCampo("Nome:", 'pessoa', "TEntry", '');
        $obFieds->setProperty('pessoa', 'disabled', 'disabled');
        $obFieds->setProperty('pessoa', 'size', '100');
        $obFieds->setValue("pessoa", $obPessoa->pessoa->nome_razaosocial);
        ...
        $obLivro = $this->getHistoricoLivro($obDevolucao->codigolivro);
        ...
        $obFieds->geraCampo("Cod.:", 'codigo', "TEntry", '');
        $obFieds->setProperty('codigo', 'disabled', 'disabled');
        $obFieds->setValue("codigo", $obLivro->livro->codigo);
        ...
        $retDevolucao = $this->setDevolucao($obDevolucao->codigolivro,$obDevolucao->codigopessoa);
        ...
        $obFieds->geraCampo("Cod.:", 'codigo', "TEntry", '');
        $obFieds->setProperty('codigo', 'disabled', 'disabled');
        $obFieds->setValue("codigo", $retDevolucao["codigo"]);
        ...
        if( $retDevolucao["multa"] != 0) {
            ...
            $botaoEdit->setProperty("onclick", "showBoleto('".$retDevolucao["codigoconta"]."')");
    ...
    */
    public function viewSetLocacao($codigolivro = "null",$codigopessoa = "null") {
        ...
        $lista->addColumn(new TDataGridColumn('codigo', 'Cod. Locação', 'center', '20%'));
    ...
    public function viewReserva($codigo){
        ...
        $infoLivro = new TDbo(TConstantes::VIEW_PATRIMONIOS_LIVROS);
        $crit = new TCriteria();
        $crit->add(new TFilter('codigopatrimonio','=',$codigo));
        ...
        if($obLivro = $retLivro->fetchObject()){
            ...
            $login = $login->getUser();
            $codigoUsuario = $login->codigopessoa;
        ...
        $situacao = $obLivro->codigolocador != '' ? 'Locado' : 'Disponível';
