    ...
    public function getInsncricao($codigoinscricao, $curso = true, $turma = true){
        ...
        try{
            ...
            $obTDbo = new TDbo(TConstantes::DBPESSOAS_INSCRICOES);
                ...
                $criteriaInscrito->add(new TFilter('codigo','=',$codigoinscricao));
            $Inscrito = $obTDbo->select('*', $criteriaInscrito);
            $obInscrito = $Inscrito->fetchObject();

                if($curso){
                   $curso = new TCurso();
                   $obInscrito->curso = $curso->getCurso($obInscrito->codigocurso);
                }
...
//                }
            ...
            $obCreditoM->setPessoa($obInscricao->codigopessoa);
            ...
            $obCreditoM->setPlanoConta($obTurma->codigoplanoconta);
            ...
            $obCreditoM->setInstrincoesPagamento($infoDesconto);
                ...
                $obProdMatricula = $prodMatricula->getProduto('codigo', $obTurma->codigoproduto);
                //==========================================
            ...
            if($codigotransacao) {
                ...
                $obTDbo = new TDbo(TConstantes::DBPESSOAS_INSCRICOES);
                    ...
                    $criteriaUpInscricao->add(new TFilter('codigo','=',$codigoinscricao));
                $obTDbo->update(array('codigotransacao'=>$codigotransacao), $criteriaUpInscricao);

                $dados['codigotrasacao'] = $codigotransacao;
                $dados['codigoconta']    = $codigoContaM;
    ...
    private function setTransacaoTaxa($codigoinscricao){
        ...
        try{
            ...
            if($obTurma->valortaxa > 0) {
                ...
                $obCredito->setPessoa($obInscricao->codigopessoa);
                ...
                $obCredito->setPlanoConta($obTurma->codigoplanoconta);
                $obCredito->setVencimento($obInscricao->vencimentotaxa, "3");

                //retorna produto taxa de inscrição=========
                $prodTaxa = new TProduto();
                $obProdTaxa = $prodTaxa->getProduto('tabela', $obTurma->codigo);
                //==========================================
        ...
        try{
            ...
            //configura transação da matrícula
	            ...
	            if($dadosTransacaoMatricula){
	                $botMatricula = "<input class=\"ui-state-default ui-corner-all ui-state-hover\" type=\"button\" name=\"impBoleto\" id=\"impBoleto\" value=\"Gerar Boleto\"  onClick=\"showBoleto('".$dadosTransacaoMatricula['codigoconta']."')\" >";
	            }
            }
            //configura conta credito da taxa de inscrição
            $dadosTransacaoTaxa = $this->setTransacaoTaxa($codigoinscricao);
            if($dadosTransacaoTaxa){
                $botTaxa = "<input class=\"ui-state-default ui-corner-all ui-state-hover\" type=\"button\" name=\"impBoleto\" id=\"impBoleto\" value=\"Gerar Boleto\" onClick=\"showBoleto('".$dadosTransacaoTaxa['codigoconta']."')\" >";
