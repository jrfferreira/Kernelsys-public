...
class TSecretaria {
    ...
    public function viewRelatorioNotas($codigoTurma) {
        ...
        try {
            if($codigoTurma) {
                ...
                $listaTurmaDisciplinas = $TTurma->getTurmaDiciplinas($obTurma->seq);
                ...
                $TAluno = new TAluno();
                	...
                	$criteriaAluno->add(new TFilter('turmseq','=',$obTurma->seq));
	                ...
	                $alunoNotas = $TAluno->listaNotas($codigoaluno);	                
	                    ...
	                    foreach($listaTurmaDisciplinas as $codigoTurmaDisciplina => $obTurmaDisciplinas){
	                    	...
	                    	foreach($alunoNotas as $obNota){
	                    		if($codigoTurmaDisciplina == $obNota->tudiseq){
            ...
            if($codigoaluno){
                ...
                foreach($getFinanceiro->transacoes as $key => $obTransacoes) {
                    foreach($obTransacoes->contas as $kayContas => $obContas) {
                        ...
                        $impressao->onclick = ($obContas->stpaseq == 1) ? "showBoleto('".$obContas->seq."')" : "alert('Não é possível imprimir o boleto.')";
     ...
     */
    public function viewTaxaOcupacao($tudiseq){
       try{
                $TTurmaDisciplina = new TTurmaDisciplinas();
                $obDisciplina   = $TTurmaDisciplina->getTurmaDisciplina($tudiseq);

                $TTurma = new TTurma();
                $obTurma = $TTurma->getTurma($obDisciplina->turmseq);
                if($obDisciplina->salaseq){
                    $dbo = new TDbo(TConstantes::DBSALA);
                    $crit = new TCriteria();
                    $crit->add(new TFilter('seq','=',$obDisciplina->salaseq));
                    $ret = $dbo->select('capacidade',$crit);
                    $obSala = $ret->fetchObject();
                }
            $obFrequencias = $this->getFrequenciaAlunos($tudiseq);
    ...
    public function viewProcessosAcademicos($codigoaluno) {
        ...
        try {
            if($codigoaluno) {
                ...
                $obFieds->geraCampo("Nº de Matricula:", 'matricula', "TEntry", '');
                $obFieds->setProperty('matricula', 'disabled', 'disabled');
                $obFieds->setValue("matricula", $obAluno->seq);
    ...
    public function viewAtendimentoSolicitacao($codigoSolicitacao){
            try{
                ...
                $dbo = new TDbo(TConstantes::VIEW_ALUNO_SOLICITACAO);
                $crit = new TCriteria();
                $crit->add(new TFilter('seq','=',$codigoSolicitacao));
                $ret = $dbo->select('*',$crit);
                $obSolicitacao = $ret->fetchObject();

                $TProcessoAcademico = new TProcessoAcademico();
                $obProcesso = $TProcessoAcademico->execProcessoAcademico($obSolicitacao->alunseq,$obSolicitacao->soliseq);
     ...
     public function viewConsolidacaoNotasFrequencias($codigo = ''){
        try {
            ...
            foreach ($obTurmaDisciplinas as $ch=>$vl) {
                if(!$listaCursos[$vl->cursseq])
                    $listaCursos[$vl->cursseq] = array('nome'=>$vl->nomecurso,'turmas'=>array());

                if(!$listaCursos[$vl->cursseq]['turmas'][$vl->turmseq])
                    $listaCursos[$vl->cursseq]['turmas'][$vl->turmseq] = array('nome'=>$vl->nometurma,'disciplinas'=> array());

                if(!$listaCursos[$vl->cursseq]['turmas'][$vl->turmseq]['disciplinas'][$vl->seq])
                    $listaCursos[$vl->cursseq]['turmas'][$vl->turmseq]['disciplinas'][$vl->seq] = $vl;
            }
