...
$.widget("ui.multiselect", {
  options: {
	...
	_create: function() {
		...
		this.availableActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><input type="text" class="search empty ui-widget-content ui-corner-all"/><a href="#" class="add-all" style="display: none">'+$.ui.multiselect.locale.addAll+'</a></div>').appendTo(this.availableContainer);
	...
	_getOptionNode: function(option) {                
                var a = String($(option).attr("onclick")).split("{");
                var b = a[1].slice(0,-1);
                var pos = b.indexOf("\"");
                var d = b.replace('\r','');
                var d = d.replace('\n','');
                ...
                var c = d.replace("this",'\''+$(option).attr("id")+'\'');

                c = c.replace(/^\s+|\s+$/g,"");
		option = $(option);
		var node = $('<li class="ui-state-default ui-element" title="'+option.text()+'" element="'+$(option).attr("id")+'"><span class="ui-icon"/>'+option.text()+'<a href="#" class="action" onclick="'+c+'"><span class="ui-corner-all ui-icon"/></a></li>').hide();
