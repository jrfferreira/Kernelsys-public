...
$.extend(Calculator.prototype, {
	...
	_resultClass: 'ui-widget-content', // The name of the calculator result marker class
	_focussedClass: 'ui-state-focus', // The name of the focussed marker class
		...
		for (var i = 0; i < layout.length; i++) {
			...
			for (var j = 0; j < layout[i].length; j += 2) {
				...
				html += (def[1] == this.space ? '<span class="calculator-' + def[3] + '"></span>' :
					...
					// Control buttons
					(def[1] == this.control ? ' class="ui-state-default' +
					...
					' disabled="disabled"' : '') + ' class="ui-widget-content' :
					// Binary operations
					(def[1] == this.binary ? ' class="ui-state-default' :
					// Unary operations
