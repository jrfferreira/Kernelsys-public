    ...
    public function viewCurso($codigo) {
        ...
        if($obCurso['turmas'] != NULL) {
            ...
            foreach($obCurso['turmas'] as $turma) {
                ...
                $tempCell->align = "left";
                $a = new TElement("a");
                $a->href = "TSetInscricao.class.php?tipo=i&prod=".$turma['codigoproduto']."&estilo=".$_GET['estilo'];
                $a->target = "_blank";
                $a->add("Inscreva-se");
        ...
        $tempCell->colspan = "4";
        $a = new TElement("a");
        $a->href = "TSetInscricao.class.php?tipo=d&prod=".$obCurso['codigo']."&estilo=".$_GET['estilo'];
        $a->add("Inscreva-se");
        $tempCell = $infoTurma->addCell($a);
        $tempCell->align = "right";
    ...
    */
    public function viewLista($unidade, $codigo,$tipo=null) {
        ...
        foreach ($cursos as $obRetCursos){
            ...
            $a = new TElement("a");
            $a->href = "?tipo=informacaoCurso&codigo=".$obCurso['codigo']."&estilo=".$_GET['estilo'];
            $a->add("Saiba mais");
