...
// codifica a forma de saidad dos dados no header (tratando caracteres especiais) \\
	...
	//TTransaction::close();
		...
		//if($conn = TTransaction::get()){
            ...
            $contasQuery = $sqlContas->select("codigo,nomeConta,tipoConta,tipoCusto");

            //$sqlContas = "SELECT codigo,nomeConta,tipoConta,tipoCusto FROM dbtransacoes_contas";
			//$contasQuery = $conn->Query($sqlContas);
			
				
			while ($obContas = $contasQuery->fetchObject()){
			$vetCtNome[$obContas->codigo] = $obContas->nomeConta;
			$vetCtTipo[$obContas->codigo] = $obContas->tipoConta;
			$vetCtCusto[$obContas->codigo] = $obContas->tipoCusto;
			$vetContas[$obContas->codigo] = 0;
			}	
            $sqlCaixa = new TDbo(TConstantes::DBCAIXA);
            $caixaQuery = $sqlCaixa->select("codigoconta,valorpago,tipomovimentacao,datacad,dataPag",$critCaixa);

			//$sqlCaixa = "SELECT codigoconta,valorpago,tipomovimentacao,datacad,dataPag FROM dbcaixa where ".$sqlData;
			//$caixaQuery = $conn->Query($sqlCaixa);
				
				while($obCaixa = $caixaQuery->fetchObject()){
					
						$vetContas[$obCaixa->codigoconta] = $vetContas[$obCaixa->codigoconta] + $obCaixa->valorpago;
